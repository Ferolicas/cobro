...
export async function POST(request: Request) {
  try {
    ...
    await notifyMasters({
      ...
      message: `${liquidation.collector?.name ?? user.name} cerró con una diferencia de S/ ${(Number(liquidation.differenceCents) / 100).toFixed(2)}`,
      entityType: "liquidation",
      entityId: liquidation.id,
      actionUrl: "/app/liquidaciones",
      details: {
        cobrador: liquidation.collector?.name ?? user.name,
        fecha: input.date,
