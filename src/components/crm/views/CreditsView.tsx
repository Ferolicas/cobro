...
export function CreditsView({ user, currency, initialId, refreshKey }: { user: AppUser; currency: Currency; initialId?: string; refreshKey: number }) {
  ...
  const selectedPlan = selected ? installmentLedger({
    installments: selected.installments,
    payments: selected.payments ?? [],
    noPaymentActivities: selected.activities,
    totalDueCents: selected.totalDueCents,
    installmentCents: selected.installmentCents,
  }) : [];
  ...
  return <div className="page-stack">
    ...
    {selected && !paymentOpen && !renewOpen && <Modal title={selected.client.name} subtitle={`${selected.code} · ${selected.status}`} onClose={() => setSelected(null)} wide><div className="credit-detail">
      ...
      <section className="detail-section"><h3><CalendarDays />Historial real de {selected.installmentCount} cuotas · domingos no cuentan</h3><div className="installment-grid">{selectedPlan.map((installment) => <div key={installment.number} className={installment.visualStatus} title={installment.visualStatus === "partial" ? "Se conserva exactamente lo recibido y el faltante pasa a la cuota siguiente" : installment.visualStatus === "paid-late" ? "Cuota pagada con atraso" : undefined}><span>{installment.number}</span><strong>{currency.money(installment.displayCents)}</strong><small>{shortDate(installment.dueDate)}</small>{installment.checked ? <CheckCircle2 /> : null}</div>)}</div></section>
      <section className="detail-section"><h3>Pagos registrados y justificantes</h3>{selected.payments?.map((payment) => <div className="payment-record" key={payment.id}><div className="payment-row"><span className="payment-icon"><CircleDollarSign /></span><div><strong>{payment.source === "ADVANCE_INSTALLMENT" ? "Primera cuota registrada" : payment.source === "RENEWAL_SETTLEMENT" ? "Liquidación por renovación" : "Cuota registrada"}</strong><small>{dateTime(payment.paidAt)} · {payment.method}{payment.collector?.name ? ` · Cobrador: ${payment.collector.name}` : ""}</small></div><strong>{currency.money(payment.amountCents)}</strong></div>{payment.documents?.map((document) => <a className="payment-proof" href={`/api/documents/${document.id}`} target="_blank" key={document.id}><FileUp /><span>{document.label || document.fileName}</span><small>Ver justificante</small></a>)}</div>)}</section>
