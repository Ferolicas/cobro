...
export function CreditsView({ user, currency, initialId, refreshKey }: { user: AppUser; currency: Currency; initialId?: string; refreshKey: number }) {
  ...
  const eligibleClients = clients.filter((client) => !client.credits.some((credit) => ["ACTIVE", "OVERDUE"].includes(credit.status)));
  ...
  function startLoanForm(mode: "create" | "renew") {
    ...
    if (mode === "create") setCreateOpen(true);
  ...
  return <div className="page-stack">
    ...
    {createOpen && canOperate && <Modal title="Nuevo crédito" subtitle="Solo clientes sin crédito activo · 20% · 24 cuotas" onClose={() => setCreateOpen(false)} wide><form className="modal-form form-grid" onSubmit={create}>
      <label className="field span-2"><span>Cliente *</span><select name="clientId" required autoFocus><option value="">Selecciona un cliente</option>{eligibleClients.map((client) => <option value={client.id} key={client.id}>{client.name} · {client.businessName || client.code}</option>)}</select></label>
      {!eligibleClients.length && <div className="warning-box span-2"><RefreshCw /><span>Todos tus clientes tienen crédito activo. Abre el cliente y usa Renovar.</span></div>}
      ...
      <div className="form-actions span-2"><button type="button" className="secondary-button" onClick={() => setCreateOpen(false)}>Cancelar</button><button className="primary-button" disabled={saving || !preview || !eligibleClients.length}>{saving ? "Calculando…" : "Desembolsar crédito"}</button></div>
    </form></Modal>}
