...
import { BarChart3, KeyRound, Mail, MapPin, Plus, Power, UsersRound } from "lucide-react";
...
export function CollectorsView({ refreshKey }: { refreshKey: number }) {
  ...
  async function load() {
    ...
    setCollectors(data.collectors); setZones(data.zones); setLoading(false);
  }
  ...
  async function create(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await load(); setOpen(false); toast.success("Cobrador creado. Contraseña: cobro1234*");
  ...
  async function reset(item: Collector) {
    ...
    try { await api(`/api/collectors/${item.id}/reset-password`, { method: "POST" }); toast.success("Contraseña restablecida y sesiones cerradas"); await load(); }
    catch (error) { toast.error(error instanceof Error ? error.message : "Error"); }
  }

  async function toggle(item: Collector) {
    try { await api(`/api/collectors/${item.id}`, { method: "PATCH", body: JSON.stringify({ active: !item.active }) }); await load(); toast.success(item.active ? "Acceso desactivado" : "Acceso activado"); }
    catch (error) { toast.error(error instanceof Error ? error.message : "Error"); }
  }

  if (loading) return <LoadingState />;
  return <div className="page-stack">
    <div className="toolbar"><div className="team-summary"><span><UsersRound /></span><div><strong>{collectors.filter((collector) => collector.active).length} activos</strong><small>{collectors.length} cuentas creadas</small></div></div><button className="primary-button" onClick={() => setOpen(true)}><Plus />Nuevo cobrador</button></div>
    <div className="collector-grid">{collectors.map((item) => <article className={!item.active ? "disabled" : ""} key={item.id}>
      ...
      <button className="collector-overview-button" onClick={() => router.push(`/app/liquidaciones?collectorId=${item.id}`)}><BarChart3 /><span><strong>Ver control financiero completo</strong><small>Liquidación, M.S, semana y cadena</small></span></button>
