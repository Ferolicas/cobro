...
const masterNav = [
  { id: "dashboard", label: "Resumen", icon: LayoutDashboard }, { id: "cobro-hoy", label: "Cobro de hoy", icon: CircleDollarSign }, { id: "clientes", label: "Clientes", icon: ContactRound }, { id: "creditos", label: "Créditos", icon: CreditCard }, { id: "liquidaciones", label: "Liquidaciones", icon: ClipboardCheck }, { id: "cobradores", label: "Cobradores", icon: UsersRound }, { id: "reportes", label: "Caja y reportes", icon: WalletCards }, { id: "auditoria", label: "Auditoría", icon: BookOpenCheck },
];
const collectorNav = masterNav.filter((item) => !["cobradores", "auditoria"].includes(item.id));
...
export function CrmShell({ user, slug }: { user: AppUser; slug: string[] }) {
  const router = useRouter(); const view = slug[0] || "dashboard"; const entityId = slug[1];
  ...
  const [title, subtitle] = titleMap[view] ?? titleMap.dashboard;
  return <div className="app-shell">
    ...
    <div className="app-stage"><header className="app-header"><button className="menu-button" onClick={()=>setSidebar(true)}><Menu/></button><div className="header-search"><Search/><input placeholder="Buscar clientes o créditos…" onKeyDown={(e)=>{if(e.key==="Enter"&&e.currentTarget.value.trim())router.push(`/app/clientes?q=${encodeURIComponent(e.currentTarget.value)}`)}}/></div><div className="header-actions"><div className="currency-toggle"><button className={currency==="PEN"?"active":""} onClick={()=>setCurrency("PEN")}>S/ PEN</button><button className={currency==="COP"?"active":""} onClick={()=>setCurrency("COP")}>$ COP</button></div><button className="notification-button" onClick={()=>setNotificationsOpen(!notificationsOpen)} aria-label="Notificaciones"><Bell/>{unread>0&&<b>{unread>99?"99+":unread}</b>}</button><button className="profile-button"><span>{user.name.split(" ").slice(0,2).map((part)=>part[0]).join("").toUpperCase()}</span><div><strong>{user.name}</strong><small>{user.role==="MASTER"?"Administrador":"Cobrador"}</small></div></button></div></header>
      ...
      <main className="app-main"><div className="page-heading"><div><p>{user.role==="MASTER"?"PANEL MAESTRO":"MI RUTA"}</p><h1>{title}</h1><span>{subtitle}</span></div><div className="live-chip"><i></i>En vivo</div></div>
        {view==="dashboard"&&<DashboardView user={user} currency={currencyContext} refreshKey={refreshKey}/>} {view==="clientes"&&<ClientsView user={user} currency={currencyContext} initialId={entityId} refreshKey={refreshKey}/>} {view==="creditos"&&<CreditsView user={user} currency={currencyContext} initialId={entityId} refreshKey={refreshKey}/>} {view==="cobro-hoy"&&<TodayView currency={currencyContext} refreshKey={refreshKey}/>} {view==="liquidaciones"&&<LiquidationsView user={user} currency={currencyContext} refreshKey={refreshKey}/>} {view==="cobradores"&&user.role==="MASTER"&&<CollectorsView refreshKey={refreshKey}/>} {view==="reportes"&&<ReportsView user={user} currency={currencyContext} refreshKey={refreshKey}/>} {view==="auditoria"&&user.role==="MASTER"&&<AuditView refreshKey={refreshKey}/>} {!nav.some((item)=>item.id===view)&&<DashboardView user={user} currency={currencyContext} refreshKey={refreshKey}/>} 
      </main>
    </div>
    ...
    <nav className="bottom-nav">{collectorNav.slice(0,5).map((item)=><button key={item.id} className={view===item.id?"active":""} onClick={()=>navigate(item.id)}><item.icon/><span>{item.label.replace(" de hoy","")}</span></button>)}</nav>
  </div>;
}
